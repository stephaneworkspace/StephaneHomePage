...
namespace StephaneHomePage.Data.Astrologie.ThemeAstral.Draw
{
    public class CalcDraw
    {
        ...
        public List<Offset> lineTrigo(double angular, double radiusCircleBegin, double radiusCircleEnd)
        {
            ...
            double dx1 = getCenter().dx + Math.Cos(angular / CIRC * 2 * Math.PI) * -1 * radiusCircleBegin;
            double dy1 = getCenter().dy + Math.Sin(angular / CIRC * 2 * Math.PI) * radiusCircleBegin;
            string a = Math.Cos(angular / CIRC * 2 * Math.PI).ToString();
            string b = Math.Sin(angular / CIRC * 2 * Math.PI).ToString();
            double xxx1 = getCenter().dx;
            double xxy2 = getCenter().dy;
            returnList.Add(new Offset(dx1, dy1));
            double dx2 = getCenter().dx + Math.Cos(angular / CIRC * 2 * Math.PI) * -1 * radiusCircleEnd;
            double dy2 = getCenter().dy + Math.Sin(angular / CIRC * 2 * Math.PI) * radiusCircleEnd;
        ...
        {
            double dx = getCenter().dx + Math.Cos(angular / CIRC * 2 * Math.PI) * -1 * radiusCircle;
            double dy = getCenter().dy + Math.Sin(angular  / CIRC * 2 * Math.PI) * radiusCircle;
        ...
        public List<Offset> pathTrianglePointer(double angular, double angularPointer, double radiusCircleBegin, double radiusCircleEnd)
        {
            ...
            double dx1 = getCenter().dx + Math.Cos(angular1 / CIRC * 2 * Math.PI) * -1 * radiusCircleBegin;
            double dy1 = getCenter().dy + Math.Sin(angular1 / CIRC * 2 * Math.PI) * radiusCircleBegin;
            returnList.Add(new Offset(dx1, dy1));
            double dx2 = getCenter().dx + Math.Cos(angular2 / CIRC * 2 * Math.PI) * -1 * radiusCircleBegin;
            double dy2 = getCenter().dy + Math.Sin(angular2 / CIRC * 2 * Math.PI) * radiusCircleBegin;
            returnList.Add(new Offset(dx2, dy2));
            double dx3 = getCenter().dx + Math.Cos(angular / CIRC * 2 * Math.PI) * -1 * radiusCircleEnd;
            double dy3 = getCenter().dy + Math.Sin(angular / CIRC * 2 * Math.PI) * radiusCircleEnd; ;
